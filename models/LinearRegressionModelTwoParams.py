...
class LinearRegressionModelTwoParams(MachineLearningModel):
    ...
    def define_mathematically_ideal_params(self, data_necessity_type: DataNecessityType = DataNecessityType.TRAINING):
        ...
        if dataset_with_defined_data_necessity_type is None:
            raise ValueError("Unable to define datasets with provided data_necessity_type")
    ...
    def define_learning_data(dataset_with_defined_data_necessity_type: DataSetObject):
        ...
        Ex = sum([dataset_with_defined_data_necessity_type.input_data[0][i] for i in range(n)])
        Ey = sum([dataset_with_defined_data_necessity_type.output_data[i] for i in range(n)])
        Exx = sum([dataset_with_defined_data_necessity_type.input_data[0][i] ** 2 for i in range(n)])
        Exy = sum([dataset_with_defined_data_necessity_type.input_data[0][i]
    ...
    def update_data(self,
            training_input: Optional[List[List[float]]] = None,
            testing_input: Optional[List[List[float]]] = None,
